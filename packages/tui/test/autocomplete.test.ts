...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		...
		beforeEach(() => {
			rootDir = mkdtempSync(join(tmpdir(), "hodeuscli-autocomplete-root-"));
		...
		test("includes hidden paths but excludes .git", async () => {
			setupFolder(baseDir, {
				dirs: [".hodeuscli", ".github", ".git"],
				files: {
					".hodeuscli/config.json": "{}",
			...
			const values = result?.items.map((item) => item.value) ?? [];
			assert.ok(values.includes("@.hodeuscli/"));
	...
	describe("dot-slash path completion", () => {
		...
		beforeEach(() => {
			baseDir = mkdtempSync(join(tmpdir(), "hodeuscli-autocomplete-"));
		});
