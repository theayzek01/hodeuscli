...
import type { Api, Context, Model, OpenAICompletionsCompat } from "@games-coder/hodeuscli-ai";
import { getApiProvider } from "@games-coder/hodeuscli-ai";
import { getOAuthProvider } from "@games-coder/hodeuscli-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `hodeuscli-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
