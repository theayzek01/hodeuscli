...
import { Agent, type ThinkingLevel } from "@games-coder/hodeuscli-agent-core";
import { getModel, type Model } from "@games-coder/hodeuscli-ai";
...
describe.skipIf(!HAS_ANTIGRAVITY_AUTH)("Compaction with thinking models (Antigravity)", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `hodeuscli-thinking-compaction-test-${Date.now()}`);
...
describe.skipIf(!HAS_ANTHROPIC_AUTH)("Compaction with thinking models (Anthropic)", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `hodeuscli-thinking-compaction-anthropic-test-${Date.now()}`);
		mkdirSync(tempDir, { recursive: true });
	});
