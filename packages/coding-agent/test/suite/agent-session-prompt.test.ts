...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@games-coder/hodeuscli-ai";
...
describe("AgentSession prompt characterization", () => {
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `hodeuscli-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
