...
import { createAssistantMessageEventStream } from "@games-coder/hodeuscli-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `hodeuscli-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
