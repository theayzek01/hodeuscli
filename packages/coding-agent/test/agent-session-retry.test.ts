...
import { Agent, type AgentEvent, type AgentTool } from "@games-coder/hodeuscli-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@games-coder/hodeuscli-ai";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `hodeuscli-retry-test-${Date.now()}`);
