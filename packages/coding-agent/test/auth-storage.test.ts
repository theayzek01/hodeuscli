...
import { registerOAuthProvider } from "@games-coder/hodeuscli-ai/oauth";
...
describe("AuthStorage", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `hodeuscli-test-auth-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`);
