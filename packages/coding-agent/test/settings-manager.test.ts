...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".hodeuscli"), { recursive: true });
	});
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates Hodeuscli starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".hodeuscli", "settings.json");
	...
	describe("project settings directory creation", () => {
		it("should not create .hodeuscli folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .hodeuscli folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .hodeuscli folder that beforeEach created
			rmSync(join(projectDir, ".hodeuscli"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .hodeuscli folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".hodeuscli"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .hodeuscli folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .hodeuscli folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .hodeuscli folder should NOT exist yet
			expect(existsSync(join(projectDir, ".hodeuscli"))).toBe(false);
			...
			// Now .hodeuscli folder should exist
			expect(existsSync(join(projectDir, ".hodeuscli"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".hodeuscli", "settings.json"))).toBe(true);
		});
	});
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".hodeuscli", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
