...
Content`,
		...
		it("should auto-discover root markdown skills from .hodeuscli skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .hodeuscli", async () => {
			const extDir = join(tempDir, ".hodeuscli", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".hodeuscli", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".hodeuscli", "agent");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .hodeuscli auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".hodeuscli\n");

			const skillDir = join(tempDir, ".hodeuscli", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with Hodeuscli manifest", async () => {
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .hodeuscli settings base", () => {
			...
			const rel = relative(join(tempDir, ".hodeuscli"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in Hodeuscli manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".hodeuscli", "npm")],
				undefined,
			);
		});
		...
		it("should not fetch npm registry during resolve for installed unpinned packages", async () => {
			const installedPath = join(tempDir, ".hodeuscli", "npm", "node_modules", "example");
			mkdirSync(join(installedPath, "extensions"), { recursive: true });
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".hodeuscli", "npm", "node_modules", "example");
