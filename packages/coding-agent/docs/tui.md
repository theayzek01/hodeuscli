> hodeuscli can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@mariozechner/hodeuscli-tui`](https://github.com/badlogic/hodeuscli-mono/tree/main/packages/tui)
...
Without this propagation, tyhodeuscling with an IME (Chinese, Japanese, Korean, etc.) will show the candidate window in the wrong position on screen.
...
**In custom tools** via `hodeuscli.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = hodeuscli.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@mariozechner/hodeuscli-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@mariozechner/hodeuscli-tui";
```

### Text

Multi-line text with word wraphodeuscling.
...
} from "@mariozechner/hodeuscli-tui";
...
```typescript
hodeuscli.registerCommand("hodeusclick", {
...
```typescript
import { getMarkdownTheme } from "@mariozechner/hodeuscli";
import { Markdown } from "@mariozechner/hodeuscli-tui";
...
2. **Simple containers** - Just grouhodeuscling other components without adding themed content
...
For letting users hodeusclick from a list of options. Use `SelectList` from `@mariozechner/hodeuscli-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@mariozechner/hodeuscli";
import { DynamicBorder } from "@mariozechner/hodeuscli";
import { Container, type SelectItem, SelectList, Text } from "@mariozechner/hodeuscli-tui";
...
For operations that take time and should be cancellable. `BorderedLoader` shows a shodeusclinner and handles escape to cancel.

```typescript
import { BorderedLoader } from "@mariozechner/hodeuscli";

hodeuscli.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@mariozechner/hodeuscli-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@mariozechner/hodeuscli";
import { Container, type SettingItem, SettingsList, Text } from "@mariozechner/hodeuscli-tui";

hodeuscli.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
import { matchesKey, truncateToWidth } from "@mariozechner/hodeuscli-tui";
