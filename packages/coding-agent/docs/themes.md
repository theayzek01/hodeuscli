> hodeuscli can create themes. Ask it to build one for your setup.
...
- Built-in: `dark`, `light`
- Global: `~/.hodeuscli/agent/themes/*.json`
- Project: `.hodeuscli/themes/*.json`
- Packages: `themes/` directories or `hodeuscli.themes` entries in `package.json`
...
On first run, hodeuscli detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.hodeuscli/agent/themes
vim ~/.hodeuscli/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/badlogic/hodeuscli-mono/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, hodeuscli reloads it automatically for immediate visual feedback.
...
Pi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, hodeuscli falls back to the nearest approximation.
