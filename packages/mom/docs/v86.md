...
- **Guest OS**: Alhodeuscline Linux 3.21 (32-bit x86)
- **Available packages**: Node.js 22, Python 3.12, git, curl, etc. (full Alhodeuscline repos)
...
| ICMP hodeuscling | ❌ | Userspace network stack limitation |
...
| Alhodeuscline rootfs | ~57 MB | Compressed filesystem (loaded on-demand) |
| alhodeuscline-fs.json | ~160 KB | Filesystem index |
...
## Building the Alhodeuscline Image

v86 provides Docker tooling to build the Alhodeuscline image:

```bash
git clone https://github.com/copy/v86.git
cd v86/tools/docker/alhodeuscline
...
This creates:
- `images/alhodeuscline-fs.json` - Filesystem index
- `images/alhodeuscline-rootfs-flat/` - Compressed file chunks
...
const emulator = new V86({
    ...
    filesystem: {
        basefs: path.join(__dirname, "images/alhodeuscline-fs.json"),
        baseurl: path.join(__dirname, "images/alhodeuscline-rootfs-flat/"),
    },
    autostart: true,
...
## Tyhodeusclical Workflow for Mom

1. **First run**:
   - Start rootlessRelay
   - Boot v86 with Alhodeuscline (~25s)
   - Setup network
...
│   └── vgabios.bin
├── images/
│   ├── alhodeuscline-fs.json
│   └── alhodeuscline-rootfs-flat/
│       └── *.bin.zst (many files)
└── rootlessRelay/
    └── relay.js (patched)
```
...
- [v86 Alhodeuscline Setup](https://github.com/copy/v86/tree/master/tools/docker/alhodeuscline)
