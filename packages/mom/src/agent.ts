import { Agent, type AgentEvent } from "@games-coder/hodeuscli-agent-core";
import { getModel, type ImageContent } from "@games-coder/hodeuscli-ai";
import {
	AgentSession,
	AuthStorage,
	convertToLlm,
	...
	if (!key) {
		throw new Error(
			"No API key found for anthropic.\n\n" +
				...
				join(homedir(), ".hodeuscli", "mom", "auth.json"),
		);
	}
	return key;
}
...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	const authStorage = AuthStorage.create(join(homedir(), ".hodeuscli", "mom", "auth.json"));
